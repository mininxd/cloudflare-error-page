...
<head>
    <title>${config.title || 'Not Found'} | Error ${config.error_code || '404'}</title>
    <meta charset="UTF-8" />
    ...
    <div id="cf-error-details" class="p-0">
        <header class="mx-auto pt-10 lg:pt-6 lg:px-8 w-240 lg:w-full mb-8">
            <h1 class="inline-block sm:block sm:mb-2 font-light text-60 lg:text-4xl text-black-dark leading-tight mr-2">
                <span class="inline-block">${config.title || 'Not Found'}</span>
                <span class="code-label">Error code ${config.error_code || '404'}</span>
            </h1>
            <div>
                Visit <a href="${(config.more_information && config.more_information.link) || 'https://www.cloudflare.com/'}" target="_blank" rel="noopener noreferrer">${(config.more_information && config.more_information.text) || 'cloudflare.com'}</a> for ${(config.more_information && config.more_information.for) || 'more information'}.
            </div>
            <div class="mt-3" id="error-time"></div>
        </header>
          ...
          <div class="w-240 lg:w-full mx-auto">
                <div class="clearfix md:px-8">
                    ...
                    <div id="cf-cloudflare-status" class=" relative w-1/3 md:w-full py-15 md:p-0 md:py-8 md:text-left md:border-solid md:border-0 md:border-b md:border-gray-400 overflow-hidden float-left md:float-none text-center">
                        ...
                        <span class="md:block w-full truncate">${(config.cloudflare_status && config.cloudflare_status.location) || 'Indonesia'}</span>
                        <h3 class="md:inline-block mt-3 md:mt-0 text-2xl text-gray-600 font-light leading-1.3" style="color: #2f7bbf;">${(config.cloudflare_status && config.cloudflare_status.name) || 'Cloudflare'}</h3>
                        <span class="leading-1.3 text-2xl" style="color: #9bca3e">Working</span>
                    </div>
                    ...
                    <div id="cf-host-status" class="cf-error-source relative w-1/3 md:w-full py-15 md:p-0 md:py-8 md:text-left md:border-solid md:border-0 md:border-b md:border-gray-400 overflow-hidden float-left md:float-none text-center">
                        ...
                        <span class="md:block w-full truncate">${(config.host_status && config.host_status.location) || 'Sites'}</span>
                        <h3 id="host-name" class="md:inline-block mt-3 md:mt-0 text-2xl text-gray-600 font-light leading-1.3">Host</h3>
                        <span class="leading-1.3 text-2xl" style="color: #bd2426">${(config.host_status && config.host_status.status_text) || 'Not Found'}</span>
                    </div>
        ...
        <div class="w-240 lg:w-full mx-auto mb-8 lg:px-8">
            <div class="clearfix">
                <div class="w-1/2 md:w-full float-left pr-6 md:pb-10 md:pr-0 leading-relaxed">
                    ...
                    <p>${config.what_happened || "There is an internal server error on Cloudflare's network."}</p>
                </div>
                <div class="w-1/2 md:w-full float-left leading-relaxed">
                    <h2 class="text-3xl font-normal leading-1.3 mb-4">What can I do?</h2>
                    <p>${config.what_can_i_do || 'Please try again in a few minutes.'}</p>
                </div>
            </div>
        </div>
        ...
        <div class="cf-error-footer cf-wrapper w-240 lg:w-full py-10 sm:py-4 sm:px-8 mx-auto text-center sm:text-left border-solid border-0 border-t border-gray-300">
            <p class="text-13">
                <span class="cf-footer-item sm:block sm:mb-1">Ray ID: <strong class="font-semibold" id="ray-id"></strong></span>
                ...
                <span class="cf-footer-item sm:block sm:mb-1"><span>Performance &amp; security by</span> <a rel="noopener noreferrer" href="${(config.perf_sec_by && config.perf_sec_by.link) || 'https://dns.mininxd.xyz/'}" id="brand_link" target="_blank">${(config.perf_sec_by && config.perf_sec_by.text) || 'Mininxd Network'}</a></span>
            </p>
        </div><!-- /.error-footer -->
    </div>
</div>
...
(function() {
    // Helper to format time as "YYYY-MM-DD HH:MM:SS UTC"
    ...
    // Helper to generate a 16-digit hex UUID-like string
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        const hostStatus = cfg.host_status || {};

        // 1. Set time
        const timeElement = document.getElementById('error-time');
        if (timeElement) {
            if (cfg.time) {
                timeElement.textContent = cfg.time;
            } else {
        ...
        if (rayIdElement) {
            if (cfg.ray_id) {
                rayIdElement.textContent = cfg.ray_id;
            } else {
                rayIdElement.textContent = generateRayId();
            }
        }

        // 3. Set Host Name (window.location.origin)
        const hostNameElement = document.getElementById('host-name');
        if (hostNameElement) {
            if (hostStatus.name) {
                hostNameElement.textContent = hostStatus.name;
            } else {
                hostNameElement.textContent = window.location.origin;
            }
        }
        ...
        if (ipElement) {
            if (cfg.client_ip) {
                 ipElement.textContent = cfg.client_ip;
            } else {
                fetch('https://api-mininxd.vercel.app/ip')
                    .then(response => response.json())
                    .then(data => {
                        if (data && data.ip && data.ip.ipv4) {
                            ipElement.textContent = data.ip.ipv4;
                        } else {
        ...
        if (b && c) {
            ...
            c.addEventListener("click", function() {
                ...
                var ipSpan = document.getElementById("cf-footer-ip");
                if (ipSpan) ipSpan.classList.remove("hidden");
            });
        }
    });
})();
